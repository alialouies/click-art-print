...
function Photography(props) {
  ...
  useEffect(() => {
    axios
      .get(
        "https://www.rijksmuseum.nl/api/nl/collection?key=eQKm0Vx6&culture=en&involvedMaker=Friedrich%20Carel%20Hisgen&f.dating.period=19&imgonly=True"
      )

      .then((response) => response.data)
      .then((data) => {
        const dataArray = [];
        data.artObjects.map((object) =>
          dataArray.push([
            object.webImage.url,
            object.title,
